...
def print_player_data(dynamo_result, total_duration=None):
    ...
    print('Consumed Capacity: {}'.format(dynamo_result['ConsumedCapacity']['CapacityUnits']))
...
if __name__ == "__main__":
    ...
    player_game_data = score_table.query(
        ...
        KeyConditionExpression=Key('badge').eq('Champs'),
        ReturnConsumedCapacity='TOTAL')

    end_time = time.time()

    duration = int((end_time - start_time) * SECONDS_TO_MILIS)
    print_player_data(player_game_data, total_duration=duration)