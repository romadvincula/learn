...
try: 
  ...
  json_templates_path    = f"{workspace_src_path}/Includes/tools/jinja_templates/"
...
# Print out details of the job to user
print(f"""
Job Name:            {job_name}
Job Type:            {workflow_type}
{compute}
...
def generate_job():
  if sku[0] == 'DLT':
    ...
    print(f"Rendering Spark Declarative Pipeline JSON via jinja template located at {jinja_template_path}")
    rendered_pipeline_dag = generate_dag(jinja_template_path, dag_args)
    print("Submitting rendered Spark Declarative Pipeline JSON to Databricks Pipelines API")
  ...
  print(f"Rendering New Job JSON via jinja template located at {jinja_template_path}")
  rendered_workflow_dag = generate_dag(jinja_template_path, dag_args)  
  print("Submitting rendered Job JSON to Databricks Jobs API")
...
job_id = generate_job()
url = f"/#job/{job_id}"
displayHTML(f"<h1><a href={url}>Your Lakeflow Job can be found here!</a></h1>")