...
def extractor():
    ...
    def checks():
        ...
        validate_cocktail_fields = PythonOperator(
            task_id='validate_cocktail_fields',
            python_callable=_validate_cocktail_fields,
            on_failure_callback=_handle_empty_size
        )
        # define dependencies within this task group
        check_size >> validate_cocktail_fields  
    ...
    @task(trigger_rule=TriggerRule.NONE_FAILED_MIN_ONE_SUCCESS,
          templates_dict={
              'the_current_date': '{{ ds }}',
              'my_api': '{{ var.value.api }}' # avoid using Variable.get in runtime
              })
    def clean_data(templates_dict):
        ...
        print(f'Data cleaned for the data { templates_dict['the_current_date'] }')
